...
void GSCircleHollow(struct SDL_Circle position,float red, float green, float blue, float alpha)
{
	float x1, y1, radius;
	int i;
	...
	glBegin(GL_LINE_LOOP);
		...
		for( i = 0;i <= 360;i+=CIRCLE_QUALITY)
		{
			glVertex2f(x1 + sin(i*PI/180) * radius, y1 + cos(i*PI/180) * radius);
...
void GSCircle (struct SDL_Circle position,float red, float green, float blue, float alpha)
{
	float originX;
	float originY;
	...
	originX = (float) position.centerX;
	originY = (float) position.centerY;
	...
	for(i = 0;i <= 360; i+=CIRCLE_QUALITY)
	{
		...
		vectorX=originX+(position.radius*(float)sin((double)angle));
		vectorY=originY+(position.radius*(float)cos((double)angle));		
...
void GSRect( SDL_Rect position, float red, float green, float blue, float alpha )
{
 
	//#ifndef USING_GLSDL

	//printf("not using glsdl...\n");
...
void GSLine( float X1,float Y1,float X2,float Y2, float red, float green, float blue, float alpha )
{
